        ...
        if not genome_record:
            ...
            for row in existing_data:
                ...
                if locus_tag in target_locus_tags and locus_tag in cds_by_locus_tag:
                    ...
                    # ターゲット遺伝子の開始・終了位置を取得
                    # GenBank Featureのlocationは0-basedだが、Biopythonは自動的に処理してくれる
                    # ただし、スライス操作は0-basedであるため、直接start/endを使用
            ...
            if processed_data:
                ...
                print(f"成功: フランキング領域情報が '{output_tsv_path}' に出力されました。")
            else:
                print(f"情報: 処理するデータがありませんでした。")
